export interface CardSearchResult {
  ...
  color?: string;
  card_type?: string;
  all_editions?: { name: string, code?: string }[];
}
  ...
  try {
    if (game === 'magic') {
      ...
      if (searchData.data && searchData.data.length > 0) {
        const editions = searchData.data.map((c: any) => ({ name: c.set_name, code: c.set }));
        // Filtrer les doublons d'éditions
      ...
      if (data.data && data.data.length > 0) {
        const editions = data.data.map((c: any) => ({ name: c.set.name }));
        const uniqueEditions = editions.filter((v: any, i: any, a: any) => a.findIndex((t: any) => t.name === v.name) === i);
